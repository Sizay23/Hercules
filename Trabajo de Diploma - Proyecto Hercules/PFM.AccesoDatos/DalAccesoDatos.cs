...
namespace DAL
{
    public class DalAccesoDatos
    {
        ...
        public BEUser ObtenerUsuarioPorNombre(string username)
        {
            BEUser user = null;

            try
            {
                oCommand = new SqlCommand("Sp_Usuario_ObtenerPorNombre", oConexionDB);
                oCommand.CommandType = CommandType.StoredProcedure;
                ...
                using (SqlDataReader reader = oCommand.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        user = new BEUser
                        {
                            ...
                            DVV = reader["DW"].ToString(),
                            Nombre = reader["Nombre"].ToString(),
                            Apellido = reader["Apellido"].ToString(),
                            Mail = reader["Mail"].ToString(),
                            RolID = reader["RolID"] != DBNull.Value ? Convert.ToInt32(reader["RolID"]) : (int?)null // Conversión segura de RolID
                        };
                    }
                }
            }
